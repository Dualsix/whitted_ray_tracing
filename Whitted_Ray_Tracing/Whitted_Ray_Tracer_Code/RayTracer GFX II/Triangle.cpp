...
//para mas nota implementar intersect del triangulo
bool Triangle::Intersect( const Ray &ray, HitGeom &hitgeom ) const {
	Vec3 p = ray.origin;
	Vec3 ab = B - A;
	Vec3 ac = C - A;
	Vec3 q = ray.origin + Unit(ray.direction) *hitgeom.distance;
	Vec3 qp = p - q;
	...
	w *= ood;
	float u = 1.0f - v - w;
	hitgeom.distance = sqrt(qp.x*qp.x + qp.y*qp.y+qp.z*qp.z);
	hitgeom.normal = Unit(n*Epsilon);
	hitgeom.origin = ray.origin;
	hitgeom.point = Vec3 (A*u + B*v + C*w);
	return 1;
	}
